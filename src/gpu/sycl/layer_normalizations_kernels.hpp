...
struct layer_normalization_fwd_kernel_vec_t {
    ...
    [[sycl::reqd_sub_group_size(32)]] void operator()(
        ...
        size_t in_base_idx = (wi_offset_t + wg_offset_t) * conf_.block_size;
...
private:
    ...
    inline void compute_alg_n(int idx) const {
        ...
        if (conf_.zero_dims && conf_.calculate_stats && conf_.save_stats) {
            store_float_value(stat_md().data_type(), 0, stat_ptr(), idx);
            store_float_value(var_md().data_type(), 0, var_ptr(), idx);
        }

        unsigned flags_ = flags();
        float eps = epsilon();
        const size_t s_off = conf_.stat_d.off_l(idx);
        auto v_mean = conf_.calculate_stats
                ? 0
                : load_float_value(stat_md().data_type(), stat_ptr(), s_off);
        auto v_variance = conf_.calculate_stats
                ? 0
        ...
        if (conf_.calculate_stats && conf_.save_stats) {
            store_float_value(
                    stat_d().data_type(), v_mean, stat_out_ptr(), s_off);
            store_float_value(
                    stat_d().data_type(), v_variance, var_out_ptr(), s_off);
        }
    }
...
struct layer_normalization_bwd_kernel_vec_t {
    ...
    [[sycl::reqd_sub_group_size(32)]] void operator()(
        ...
        size_t sg_base_idx = (wg_offset_t + sg_offset_t) * conf_.block_size;
        size_t count = 0;
        dim_t start_c, start_n, end_c, end_n;
            ...
            for (dim_t c = start_c; c < end_c; ++c) {
                ...
                for (dim_t n = 0; n < (conf_.N); ++n) {
                    ...
                    auto stat_v = load_float_value(
                            stat_d().data_type(), stat_ptr(), s_off);
...
struct layer_normalization_bwd_kernel_vec_t2 {
    static constexpr int vec_len = 8;

    layer_normalization_bwd_kernel_vec_t2(
            const sycl_layer_normalization_conf_t &conf,
            sycl_in_memory_arg_t &data, sycl_out_memory_arg_t &diff_data,
            sycl_in_memory_arg_t &scale, sycl_out_memory_arg_t &diff_scale,
            sycl_out_memory_arg_t &diff_shift, sycl_in_memory_arg_t &stat,
                ...
                for (dim_t c = 0; c < conf_.C; ++c) {
                    ...
                    dd_gamma_x += (dd * gamma
                            * (load_float_value(data_md().data_type(),
                                       data_ptr(), src_off)
                                    - load_float_value(stat_d().data_type(),
                                            stat_ptr(), s_off)));
                }
