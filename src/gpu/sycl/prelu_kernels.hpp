...
struct prelu_bwd_kernel_vec_t {
    ...
    prelu_bwd_kernel_vec_t(const sycl_prelu_conf_t &conf,
            sycl_in_memory_arg_t &data,sycl_in_memory_arg_t &diff_data, sycl_in_memory_arg_t &weights,sycl_in_memory_arg_t &diff_weights,
            sycl_out_memory_arg_t &diff_dst,sycl_out_memory_arg_t &dst)
        : conf_(conf), data_(data),diff_data_(diff_data), weights_(weights),diff_weights_(diff_weights), diff_dst_(diff_dst),dst_(dst) {}
    ...
    [[sycl::reqd_sub_group_size(32)]] void operator()(
        ...
        switch (conf_.bcast_type) {
          case broadcasting_strategy_t::scalar:
               calculate_scalar(data_ptr(), weights_ptr(), diff_weights_ptr(), diff_dst_ptr(),diff_data_ptr(),ithr);
               reduce_scalar(diff_weights_ptr(),ithr);
               break;
          case broadcasting_strategy_t::no_broadcast:
               calculate_no_broadcast(data_ptr(), weights_ptr(), diff_weights_ptr(), diff_dst_ptr(),diff_data_ptr(),ithr);
               break;
          ...
          case broadcasting_strategy_t::shared_axes:
               calculate_shared_axes(data_ptr(), weights_ptr(), diff_weights_ptr(), diff_dst_ptr(),diff_data_ptr(),ithr,item);
               break;
    ...
    float ker(const float *src, const float *weights,
        const float *diff_dst, float *diff_src, dim_t data_off,
        dim_t weight_off) const {

        float src_val = load_float_value(data_md().data_type(), data_ptr(), data_off);
        float diff_dst_val = load_float_value(diff_dst_md().data_type(), diff_dst_ptr(),data_off);
        float weights_val = load_float_value(weights_md().data_type(), weights_ptr(), weight_off);

        float diff_src_res = ::dnnl::impl::math::relu_bwd_use_dst(diff_dst_val, src_val, weights_val);
        float diff_weight_res = src_val > 0 ? 0 : (diff_dst_val * src_val);
        store_float_value(data_md().data_type(), diff_src_res, diff_src, data_off);
    ...
    void reduce_scalar(float *diff_weights,size_t i)const{
       ...
       for(dim_t a=0;a<thread_count;a++){
        auto la=load_float_value(weights_md().data_type(), dst_ptr(),i);
        if(!std::isnan(la))
          c=c+1;
        }
        if(c == (thread_count) )
        {
          for(dim_t j=0;j<thread_count;j++){
            auto la=load_float_value(weights_md().data_type(), dst_ptr(),j);
            s=s+la;
          }
          store_float_value(weights_md().data_type(),s, diff_weights_ptr(),0);
        }
     }

    void set_reduction_buffers(const dim_t work_amount, dim_t &group_size, dim_t &buf_size) const {
     ...
     static dim_t weights_offset(const int mask, const sycl_md_t &mem, dims_t &dims) {
       dims_t dims_w {};
     ...
     void calculate_scalar(const float *src, const float *weights,float *diff_weights, const float *diff_dst, float *diff_src,size_t ithr) const {
        ...
        if(conf_.ndims==3){
           utils::nd_iterator_init(start, off[0], dims_d[0], off[1], dims_d[1],off[2], dims_d[2]);
         }
        if(conf_.ndims==4){
           utils::nd_iterator_init(start, off[0], dims_d[0], off[1], dims_d[1],off[2], dims_d[2], off[3], dims_d[3]);
         }
        if(conf_.ndims==5){
           utils::nd_iterator_init(start, off[0], dims_d[0], off[1], dims_d[1],off[2], dims_d[2], off[3], dims_d[3],off[4], dims_d[4]);
         }
        ...
        for (dim_t iwork = start; iwork < end; ++iwork) {
              ...
              float diff_weight_res = src_val > 0 ? 0 : (diff_dst_val * src_val);
              store_float_value(data_md().data_type(), diff_src_res, diff_src, data_off);
              //const auto res = ker(src, weights, diff_dst, diff_src, data_off, weight_off);
              s=s+diff_weight_res;
              if (++offset_buf == data_size) {
                  ...
                  group_off++;
                  s=0;
                  data_size = ((group_off + 1) * buf_size <= workload)? buf_size : workload - (group_off * buf_size);
               }
              if(conf_.ndims==1){
              ...
              if(conf_.ndims==3){
                 utils::nd_iterator_step(off[0], dims_d[0], off[1], dims_d[1],off[2], dims_d[2]);
               }
              if(conf_.ndims==4){
                 utils::nd_iterator_step(off[0], dims_d[0], off[1], dims_d[1],off[2], dims_d[2], off[3], dims_d[3]);
               }
              if(conf_.ndims==5){
                 utils::nd_iterator_step(off[0], dims_d[0], off[1], dims_d[1],off[2], dims_d[2], off[3], dims_d[3],off[4], dims_d[4]);
               }
         }
         store_float_value(weights_md().data_type(),r, dst_ptr(),ithr);
    
      }  //end of scalar

      void calculate_no_broadcast(const float *src,const float *weights, float *diff_weights, const float *diff_dst,
                                  float *diff_src,size_t ithr) const {
                const size_t nthr = conf_.n_thr;
                const dim_t work_amount = conf_.work_amount_src;
                ...
                for (dim_t iwork = start; iwork < end; ++iwork) {
                    ...
                    store_float_value(weights_md().data_type(), res, diff_weights, weight_off);
                    if(conf_.ndims==1){
                       utils::nd_iterator_step(off[0], dims_d[0]);
                     }
                    if(conf_.ndims==2){
      ...
      void calculate_shared_axes(const float *src,const float *weights, float *diff_weights, const float *diff_dst,
                                 float *diff_src,size_t ith,::sycl::nd_item<1> item)const{
           
          size_t ithr =item.get_group(0) * conf_.wg_size+item.get_local_id();
          dims_t dims_d, dims_w;
          for (int i = 0; i < max_supported_ndims; i++) {
              dims_d[i] = (data_md().dims()[i] != 0) ? data_md().dims()[i] : 1;
              dims_w[i] = (weights_md().dims()[i] != 0) ? weights_md().dims()[i] : 1;
           }
          ...
          if(conf_.ndims==3){
            utils::nd_iterator_init(start, off_w[0], dims_w[0], off_w[1], dims_w[1],off_w[2], dims_w[2]);
           }
          if(conf_.ndims==4){
            utils::nd_iterator_init(start, off_w[0], dims_w[0], off_w[1], dims_w[1],off_w[2], dims_w[2], off_w[3], dims_w[3]);
           }
          if(conf_.ndims==5){
            utils::nd_iterator_init(start, off_w[0], dims_w[0], off_w[1], dims_w[1],
          ...
          for (dim_t iwork = start; iwork < end; ++iwork) {
               ...
               for (int i = 0; i < max_supported_ndims; i++) {
                    ...
                    dims_end[i] = (dims_d[i] == dims_w[i]) ? off_w[i] + 1 : dims_d[i];
                }
               ...
               {  
                   ...
                   const auto diff_weight = ker(src, weights, diff_dst, diff_src, data_off, weight_off);
                   st=st+diff_weight;  
                   buf_off=buf_off+1;
                   if (buf_off== data_size ) {
                       group_off=group_off+1;
                       buf_off=0;
                       data_size = ((group_off + 1) * buf_size <= workload)? buf_size
                 ...
                 store_float_value(weights_md().data_type(),res, diff_weights_ptr(), weight_off);
                 if(conf_.ndims==1){
                 ...
                 if(conf_.ndims==3){
                    utils::nd_iterator_step(off_w[0], dims_w[0], off_w[1], dims_w[1],off_w[2], dims_w[2]);
                  }
                 if(conf_.ndims==4){
                    utils::nd_iterator_step(off_w[0], dims_w[0], off_w[1], dims_w[1],
                                            off_w[2], dims_w[2], off_w[3], dims_w[3]);
                  }
                 if(conf_.ndims==5){
                    utils::nd_iterator_step(off_w[0], dims_w[0], off_w[1], dims_w[1],
                                            off_w[2], dims_w[2], off_w[3], dims_w[3],off_w[4], dims_w[4]);
        ...
        sycl_in_memory_arg_t diff_data_;
        sycl_in_memory_arg_t weights_;
        sycl_in_memory_arg_t diff_weights_;
