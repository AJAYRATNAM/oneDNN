...
status_t ref_prelu_bwd_t::execute_backward(const exec_ctx_t &ctx) const {
    ...
    parallel_for(ctx, kernel_, [&](::sycl::handler &cgh) {
        ...
        auto diff_data = CTX_IN_SYCL_KERNEL_MEMORY(DNNL_ARG_DIFF_SRC);
        auto diff_weights = CTX_IN_SYCL_KERNEL_MEMORY(DNNL_ARG_DIFF_WEIGHTS);
        auto diff_dst = CTX_OUT_SYCL_KERNEL_MEMORY(DNNL_ARG_DIFF_DST);
